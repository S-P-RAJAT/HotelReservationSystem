    ...
    public static void dateValidation(String startDate, String endDate) {
        try {
            if (startDate.isEmpty() || endDate.isEmpty()) {
                throw new InvalidDateException(ExceptionType.ENTERED_EMPTY, "Please enter valid date");
            } else if (!validateFormatOfDate(endDate) || !validateFormatOfDate(startDate)) {
                throw new InvalidDateException(ExceptionType.INVALID_DATE_FORMAT, "Enter valid date order");

            } else if (dateParser(endDate).compareTo(dateParser(startDate)) <= 0) {
                throw new InvalidDateException(ExceptionType.INVALID_DATES_ORDER, "Enter valid date order");

            }

        } catch (NullPointerException e) {
            throw new InvalidDateException(ExceptionType.ENTERED_NULL, "Please enter valid date format");
        }
